...
import { EllipsisVertical, X, Plus, Calendar, Shield, Trash2, Edit, Eye, Info } from "lucide-react";
import {
  addGavgeType,
  fetchAllGavge,
  updateGavge,
...
const Gavge = () => {
  ...
  const handleSave = (e) => {
    e.preventDefault();
    if (!formState.gavgeName.trim()) return toast.error("Gavge Name is required");
  ...
  const renderActions = (gavge) => (
    <div className="relative" ref={openMenuId === gavge.id ? menuRef : null}>
      <button
        onClick={(e) => {
          e.stopPropagation();
          setOpenMenuId(openMenuId === gavge.id ? null : gavge.id);
        }}
        className={`p-3 rounded-xl transition-all duration-200 cursor-pointer ${openMenuId === gavge.id ? 'bg-accent text-white shadow-lg shadow-accent/20' : 'hover:bg-accent/5 text-[#94A3B8] hover:text-accent'}`}
      >
    ...
    <div className="space-y-4 sm:space-y-8 px-4 sm:px-8 py-4 sm:py-8 bg-[#F8FAFC] w-full min-h-screen" style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-3xl font-black text-[#0F172A] tracking-tight">Gavge-Types</h1>
          <p className="text-sm font-medium text-[#64748B]">Configure and manage system measurement protocols.</p>
        </div>
            ...
            <div className="p-8">
              <div className="flex justify-between items-center mb-10">
                <div className="space-y-1">
                  <h2 className="text-2xl font-black text-[#0F172A] tracking-tight">
                    {modalType === "add" ? "Initialize Gavge" : modalType === "edit" ? "Modify Gavge" : modalType === "view" ? "Gavge Intelligence" : "Eliminate Gavge"}
                  </h2>
                  <p className="text-[10px] font-black text-accent uppercase tracking-widest opacity-70 italic">Infrastructure Specification Node</p>
                </div>
                <button
              ...
              {(modalType === "add" || modalType === "edit") && (
                <form className="space-y-6" onSubmit={handleSave}>
                  <div>
                    <label className="block text-[10px] font-black text-[#64748B] uppercase tracking-widest mb-2 pl-1 italic">Protocol Identity</label>
                    <input
                      type="text"
                      name="gavgeName"
                      ref={nameRef}
                      ...
                      placeholder="Enter gavge name..."
                      required
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-[#64748B] uppercase tracking-widest mb-2 pl-1 italic">Format Standard</label>
                    <input
                      type="text"
                      name="gavgeFormat"
                      ...
                      onChange={handleFormChange}
                      className="w-full px-5 py-4 bg-[#F8FAFC] border-2 border-[#E2E8F0] rounded-2xl text-sm font-bold text-[#0F172A] focus:ring-accent/20 focus:border-accent transition-all outline-none shadow-sm placeholder-[#94A3B8]"
                      placeholder="Enter gavge format..."
                      required
                    />
                  </div>
                  <button
              ...
              {modalType === "view" && selectedGavge && (
                <div className="p-6 bg-[#F8FAFC] rounded-3xl border border-[#E2E8F0] space-y-6">
                  ...
                  <div className="pt-4 border-t border-[#E2E8F0] flex items-center justify-between text-[#64748B] text-xs font-bold font-mono">
                    <span>REGISTRY ID: {selectedGavge.id?.slice(-8).toUpperCase()}</span>
                    <span>LOGGED: {new Date(selectedGavge.createdAt).toLocaleDateString()}</span>
                  </div>
                </div>
              )}
              ...
              {modalType === "delete" && selectedGavge && (
                <div className="flex flex-col text-center">
                  ...
                  <h3 className="text-2xl font-black text-[#0F172A] tracking-tight mb-2">Eliminate Gavge?</h3>
